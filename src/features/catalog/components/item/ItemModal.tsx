        ...
        <div className="modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}>
            <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="item-modal-title" tabIndex={-1} className="modal-card">
                ...
                <div className="arcane-scrollbar overflow-y-auto p-6 md:p-8">
                    <div className="flex flex-wrap items-center gap-2"><RarityBadge rarity={item.rarity} />
                    ...
                    {item.properties.length > 0 && (
                        <section className="mt-8 border-t border-border pt-6">
                            ...
                            <ul className="mt-4 space-y-3 rounded border border-border bg-surface p-3">
                                {item.properties.map((property) => (
                                    <li key={property} className="flex gap-3 text-xs leading-5 text-cream">
