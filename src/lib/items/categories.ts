...
const normalizeText = (value: string) => value.normalize('NFKD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('en-US')
const containsWord = (text: string, words: readonly string[]) => words.some((word) => new RegExp(`\\b${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(text))

export function typeAbbreviation(type: unknown): string {
  return typeof type === 'string' ? type.split('|')[0].toUpperCase() : ''
}

export function resolveCategory(entity: JsonRecord, description = ''): Category {
  const name = typeof entity.name === 'string' ? entity.name : ''
  const source = typeof entity.source === 'string' ? entity.source : ''
  const override = CATEGORY_OVERRIDES[`${name.toLocaleLowerCase('en-US')}|${source.toLocaleLowerCase('en-US')}`]
  ...
  for (const [category, types] of STRUCTURED_TYPES) if (types.has(abbreviation)) return category

  const text = normalizeText(`${name} ${description}`)
  if (containsWord(text, ['amulet', 'necklace', 'periapt', 'medallion', 'brooch', 'talisman'])) return 'Amulet'
  if (containsWord(text, ['boots', 'cloak', 'gloves', 'gauntlets', 'helm', 'helmet', 'hat', 'belt', 'bracers', 'robe', 'slippers', 'goggles', 'mask'])) return 'Apparel'
  if (containsWord(text, ['bag', 'pouch', 'haversack', 'bottle', 'flask', 'bowl', 'box', 'chest', 'coffer', 'quiver']) || text.includes('portable hole')) return 'Bag/Container'
  if (text.includes('clockwork') || text.includes('mechanical device')) return 'Clockwork'
  if (containsWord(text, ['book', 'manual', 'tome', 'codex', 'grimoire'])) return 'Tome'
  if (/\b(summon|summons|summoned|conjure|conjures|materialize|creates?)\b.{0,80}\b(creature|beast|spirit|elemental|construct)\b/i.test(text)) return 'Summonable'
  if (containsWord(text, ['gem', 'pearl', 'jewel', 'crystal', 'bead', 'stone'])) return 'Gem'
  if (containsWord(text, ['hireling', 'service']) || text.includes('spellcasting service')) return 'Service'
  const miscTags = Array.isArray(entity.miscTags) ? entity.miscTags : []
  if (miscTags.includes('CNS') || /\b(consumed|destroyed|disintegrates|single use|one use)\b/i.test(text)) return 'Consumable'
  return 'Other'
}
